...
> **Curated collections of skills organized by role and expertise level.** Don't know where to start? Pick a scope below to get a curated set of skills for your role.

> These packs are curated starter recommendations for humans. Generated scope ids in `data/scopes.json` are broader catalog/workflow groupings and do not need to map 1:1 to the editorial packs below.

> **Important:** scopes are installable plugin subsets and activation presets, not invokable mega-skills such as `@web-wizard` or `/essentials-scope`. Use the individual skills listed in the pack, install the scope as a dedicated marketplace plugin, or use the activation scripts if you want only that scope's skills active in your live Antigravity directory.

> **Plugin compatibility:** root plugins and scope plugins only publish plugin-safe skills. If a scope shows `pending hardening`, the skills still exist in the repository, but that scope is not yet published for that target. `Requires manual setup` means the scope is installable, but one or more included skills need an explicit setup step before first use.
...
2. **Choose your scope** from the list below based on your role or interests.

3. **Use scope plugins or individual skills** in your AI assistant:
   - Claude Code: install the matching marketplace scope plugin, or invoke `>> /skill-name help me...`
   - Codex CLI / Codex app: install the matching scope plugin where plugin marketplaces are available, or invoke `Use skill-name...`
   - Cursor: `@skill-name` in chat
   - Gemini CLI: `Use skill-name...`

If you want a scope to behave like a focused active subset instead of a full install, use:
...
- **Claude Code**: install a scope plugin or use `>> /skill-name help me...`
- **Codex CLI**: install a scope plugin where marketplaces are available, or use `Use skill-name...`
- **Cursor**: `@skill-name` in chat
- **Gemini CLI**: `Use skill-name...`
...
_For designing and reviewing REST/GraphQL APIs, contracts, and integration layers._
...
_For schema design, query optimisation, migrations, and data integrity._
...
_For CI/CD pipelines, containerisation, IaC, secrets management, and incident response._
...
_For React/TypeScript component architecture, accessibility, Core Web Vitals, and design systems._
...
_For multi-region design, IAM, HA/DR, Well-Architected reviews, and cost optimisation._
...
_For service architecture, async patterns, message queues, caching, and resilience._
...
_For prompt engineering, RAG architecture, LLM evaluation, AI safety, and agent orchestration._
...
_Last updated: 2026 | Total Skills: 1,595+ | Total Scopes: 59+ (including 7 Engineering Agent Scopes)_
