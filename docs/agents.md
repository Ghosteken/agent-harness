...
| [api-developer](../agents/api-developer.md) | Senior API Engineer | REST/GraphQL design, contract review, auth, versioning |
| [db-architect](../agents/db-architect.md) | Senior Database Architect | Schema design, indexing, migrations, query optimisation |
| [senior-devops](../agents/senior-devops.md) | Senior DevOps Engineer | CI/CD pipelines, IaC, containerisation, incident response |
| [senior-frontend-engineer](../agents/senior-frontend-engineer.md) | Senior Frontend Engineer | React/TS components, accessibility, Core Web Vitals |
...
- "Review this Dockerfile and pipeline" → invoke `senior-devops` directly
- "Review this React component for accessibility" → invoke `senior-frontend-engineer` directly
- "Review this cloud architecture for a multi-region service" → invoke `senior-cloud-architect` directly
- "Review this background job consumer" → invoke `senior-backend-engineer` directly
- "Review this RAG pipeline for safety issues" → invoke `ai-ml-engineer` directly
...
/ship
  ...
  ├── (parallel) api-developer          → contract review (when API changes present)
  └── (parallel) senior-backend-engineer → resilience review (when service changes present)
                  ↓
