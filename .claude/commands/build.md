...
3. **Plan if needed.** If there is no `tasks/plan.md`, invoke agent-harness:planning-and-task-breakdown to generate one.
...
6. **Stop and ask the user** (do not push through) when:
   ...
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → follow agent-harness:doubt-driven-development and get explicit sign-off before continuing
